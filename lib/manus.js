 ...
 * deep-link that opens the Manus webapp directly on the file preview.
 *
 * Sandbox path is reverse-engineered from the Manus CDN URL — the file's URL
 * pathname is a `_`-joined sequence of base64 chunks whose tail (after
 * dropping the first chunk) base64-decodes to `/home/ubuntu/.../<file>`. The
 * Manus webapp uses this exact path as `previewSandboxPath`, so we append
 * the file extension and pass it through.
 */
export function collectAttachmentsWithPreview(messages, taskId) {
  ...
  for (let i = 0; i < list.length; i++) {
    ...
    for (const attachment of inner.attachments) {
      ...
      const sandboxPath = decodeSandboxPath(attachment.url, attachment.filename)
      seen.set(attachment.url, {
        filename: attachment.filename ?? 'attachment',
        url: attachment.url,
        contentType: attachment.content_type ?? null,
        type: attachment.type ?? 'file',
        preview_url: taskId
          ? buildPreviewUrl(taskId, { eventId: previewEventId, sandboxPath })
          : null,
      })
    }
  }
...
/** File extension for the previewSandboxPath query (Manus keeps the dot). */
function previewExt(filename, contentType) {
...
/**
 * Reverse-engineer the Manus sandbox path from a CDN attachment URL.
 * The pathname's last segment is `<prefix>_<chunk1>_<chunk2>_...` where each
 * chunk after the first is base64-encoded UTF-8. Concatenating those decoded
 * chunks yields the full sandbox path (e.g. `/home/ubuntu/x/y.csv`). The
 * decoded tail always ends with the filename, so we re-append the matching
 * extension to form a path the Manus webapp recognizes.
 */
function decodeSandboxPath(cdnUrl, filename, contentType) {
  try {
    const u = new URL(cdnUrl)
    const lastSeg = u.pathname.split('/').pop() ?? ''
    const encoded = lastSeg.split('.')[0] // strip extension
    const chunks = encoded.split('_').slice(1)
    let decoded = ''
    ...
    // Some sandbox paths start with garbage bytes (control or invalid-UTF8
    // replacement chars). Find the first `/` and start the path there.
    const slash = decoded.indexOf('/')
    if (slash > 0) decoded = decoded.substring(slash)
    if (!decoded) return null
    // Ensure the path ends with the filename's extension (Manus expects it).
    if (filename && !decoded.endsWith(filename)) {
      decoded = decoded.replace(/\.[^./]+$/, '') + previewExt(filename, contentType)
    }
    return decoded
  } catch {
    return null
