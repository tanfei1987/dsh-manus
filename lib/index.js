...
export const MANUS_GUIDANCE = '本机已安装 dsh-manus 插件（Manus 集成）：侧边栏「Manus」入口；用你自己的 Manus API Key 通过官方 API（open.manus.im v2）调用 Manus。能力：manus_run 把任务指令发给 Manus 并等待拿回最终答复与附件；manus_create 创建异步任务；manus_poll 轮询状态；manus_result 取回完整结果并把附件下载到本机；manus_reply 多轮追问/回答 Manus 的提问；manus_tasks 列出最近任务；manus_stop 停止任务；manus_credits 查剩余积分；manus_config 查看/设置 API Key（存 ~/.dsh/manus.json，权限 0600，也可用环境变量 MANUS_API_KEY）。重要限制：Manus 任务真实消耗用户账户积分（如 4000 积分订阅），执行大任务前先 manus_credits 确认余额；任务较长时优先 manus_create + manus_poll 异步跟进，避免长时间阻塞；附件下载目录需用户确认。用户提到「Manus / 用 Manus 帮我做 / 让 Manus 调研」时即指本插件，请据此协作。'
