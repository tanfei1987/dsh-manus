...
async function settleResult(client, taskId, { wait, timeoutMs, outputDir }) {
  ...
  const attachments = await maybeDownload(client, collectAttachments(summary), outputDir)
...
export function manusResultTool(client) {
  return defineTool({
    ...
    output: { schema: resultOutputSchema, render: renderResult },
    async execute(args) {
      const messages = await client.listMessages(args.task_id, { limit: 200, order: 'desc' })
      const summary = summarizeMessages(messages.messages ?? [])
      const attachments = await maybeDownload(client, collectAttachments(summary), args.output_dir)
