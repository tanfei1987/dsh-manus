...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = [
      ...
      ".dsh-manus-attach-link { font-size: 12px; color: var(--dsw-alias-state-business-primary, #3b82f6); text-decoration: none; overflow-wrap: anywhere; }",
    ...
    function mountPanel(controller, container) {
      ...
      function showResult(taskId, summary, messages) {
        ...
        (messages || []).forEach(function (m) {
          if (m.type === "assistant_message" && m.assistant_message && m.assistant_message.attachments) {
            (m.assistant_message.attachments || []).forEach(function (a) {
              if (a && a.url) attachments.push({ filename: a.filename || "attachment", url: a.url });
            });
          }
        });
        var seen = new Set();
        attachments.forEach(function (a) {
          ...
          seen.add(a.url);
          var link = el("a", "dsh-manus-attach-link", "⬇ " + a.filename);
          link.href = "/api/dsh-manus/attachment" + qs({ url: a.url, filename: a.filename });
      ...
      function pollOnce(taskId) {
        return api("/api/dsh-manus/task/messages" + qs({ task_id: taskId, limit: 100 })).then(function (body) {
          ...
          showResult(taskId, summary, messages);
          return summary;
        });
      }
      ...
      function loadTask(taskId) {
        ...
        api("/api/dsh-manus/task/messages" + qs({ task_id: taskId, limit: 200 })).then(function (body) {
          ...
          showResult(taskId, null, body.messages || []);
        });
      }
